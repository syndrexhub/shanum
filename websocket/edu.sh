...
# Link Hosting Kalian
wisnuvpn="raw.githubusercontent.com/wisnucokrosatrio/wisnucokrosatrio/main/websocket"
...
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/usr/bin/python -O /usr/local/bin/ws-tls 70
Restart=on-failure
...
AmbientCapabilities=CAP_NET_ADMIN CAP_NET_BIND_SERVICE
NoNewPrivileges=true
ExecStart=/usr/bin/python -O /usr/local/bin/wstunnel 591
Restart=on-failure
